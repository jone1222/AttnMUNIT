...
class AttnMUNIT_Trainer(nn.Module):
    def __init__(self, hyperparameters,d_network='MsImage',attn_type='ConSty',concat_type='add',use_self_attention=False,use_adain_each=False):
        ...
        self.gen_a = nn.Module()
        self.gen_b = nn.Module()

        self.dis_a = nn.Module()
        self.dis_b = nn.Module()
        ...
        elif self.attn_type == 'AdaINAttn':
            self.gen_a = AdaINAttnGen(hyperparameters['input_dim_a'], hyperparameters['gen'])  # auto-encoder for domain a
            self.gen_b = AdaINAttnGen(hyperparameters['input_dim_b'], hyperparameters['gen'])  # auto-encoder for domain b
        ...
        if d_network == 'SelfAttn':
            self.dis_a = SelfAttnDis(hyperparameters['input_dim_a'], hyperparameters['dis'],sa_layer=[4])
            self.dis_b = SelfAttnDis(hyperparameters['input_dim_a'], hyperparameters['dis'], sa_layer=[4])
        else:
    ...
    def gen_update(self, x_a, x_b, hyperparameters):
        ...
        s_a, s_b = self.get_random_style_code(display_size)

        #random style code for mode seeking loss
        # s_a_2, s_b_2 = self.get_random_style_code(display_size)
    ...
    def sample(self, x_a, x_b):
        ...
        # AttnGen Style Shape --> x.size(0) x 256 x 16 x 16
        s_a2, s_b2 = self.get_random_style_code(x_a.size(0))
    ...
    def dis_update(self, x_a, x_b, hyperparameters):
        ...
        # AttnGen Style Shape --> x.size(0) x 256 x 16 x 16
        s_a,s_b = self.get_random_style_code(x_a.size(0))
...
#
# class AdaINAttnMUNIT_Trainer(nn.Module):
